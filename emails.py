...
def create_template(part_name, id_code):
    ...
    More detail about the survey is provided in the online consent (link below). You can also feel free to get in touch by emailing the Study Coordinator at anxiety@pitt.edu if you have any questions or would like to speak to a researcher directly.
    Just like the last survey, to keep your responses strictly confidential, we are sending all participants a personalized study link and ID code. Authorized individuals from the CNMA Lab are the only people who will have access to your name, contact information, and any other personal information. We keep this information in a secure and password protected electronic file that is not shared with anyone else.
    ...
    If you no longer want to be contacted for this study, please contact the Study Coordinator at anxiety@pitt.edu.
...
def full_path(ids):
    ...
    for j, row in enumerate(ids.iterrows()):
